...
const OLD_BASE_URL = 'https://ufnvbcgenocuydzzoxja.supabase.co/storage/v1/object/public/images/public/';
...
async function updateDocuments(urlMap) {
  ...
  for (const doc of projects.documents) {
    if (doc.image && urlMap[doc.image]) {
      await databases.updateDocument(DB, 'projects', doc.$id, { image: urlMap[doc.image] });
  ...
  for (const doc of posts.documents) {
    if (doc.thumbnail && urlMap[doc.thumbnail]) {
      await databases.updateDocument(DB, 'blog_posts', doc.$id, { thumbnail: urlMap[doc.thumbnail] });
